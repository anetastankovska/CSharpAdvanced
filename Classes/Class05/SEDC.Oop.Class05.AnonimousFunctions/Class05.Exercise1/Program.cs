...
using Class05.Exercise1.Enums;
using Class05.Exercise1.Models;
...
List<string> firstNames = people.Where(x => x.FirstName.StartsWith("R"))
						.OrderByDescending(x => x.FirstName)
						.Select(x => x.FirstName).ToList();
firstNames.ForEach(x => Console.WriteLine(x));


Console.WriteLine("Find and print all brown dogs names and ages older than 3 years, ordered by Age - ASCENDING ORDER");
List<Dog> brownDogs = dogs.Where(x => x.Color == "Brown" && x.Age > 3)
					.OrderBy(x => x.Age).ToList();

brownDogs.ForEach(x => Console.WriteLine(x.Name));


Console.WriteLine("Find and print all persons with more than 2 dogs, ordered by Name - DESCENDING ORDER");
List<Person> peopleWithDogs = people.Where(x => x.Dogs.Count > 2)
						   .OrderByDescending(x => x.FirstName).ToList();

peopleWithDogs.ForEach(x => Console.WriteLine(x.FirstName));


Console.WriteLine("Find and print all Freddy`s dogs names older than 1 year");
people.FirstOrDefault(x => x.FirstName == "Freddy")
						.Dogs.Where(x => x.Age > 1).ToList().ForEach(x =>
						{
...
Dog nathansDog = people.FirstOrDefault(x => x.FirstName == "Nathen")
                            .Dogs.FirstOrDefault();
...
var dogNames = people.Where(x => x.FirstName == "Cristofer" || x.FirstName == "Freddy" || x.FirstName == "Erin" || x.FirstName == "Amelia")
	                 .SelectMany(x => x.Dogs)
